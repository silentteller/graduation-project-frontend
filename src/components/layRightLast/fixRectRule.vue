<template>
  ...
  <!-- <div class="BaseChart">
    <el-scrollbar style="height: 100%">
      <div id="fixRectRuleChart"></div>
    </el-scrollbar>
  </div> -->
  <div class="BaseChart">
    <el-scrollbar style="height: 100%">
      <div id="fixRectRuleChart" ref="chart" @click="openDialog()"></div>
    </el-scrollbar>
    <el-dialog title="提示" :visible.sync="dialogVisible" width="50%">
      <div id="fixRectRuleChart2" ref="chart2"></div>
    </el-dialog>
  </div>
</template>
  ...
  mounted() {
    ...
    this.$Bus.$on("sendFixPathData", (data) => {
      ...
      for (var i = 0; i < selList.length; ++i) {
        selRuleData.push(data.ruleData[selList[i]]);
        predictData.push(data.predictData[selList[i]]);
      }
      this.ruleData = selRuleData;
      this.predictData = predictData;
      this.data = data.data;
      this.draw("fixRectRuleChart", "chart");
    });
  },
  methods: {
    openDialog() {
      ...
      this.$nextTick((_) => {
        this.draw("fixRectRuleChart2", "chart2");
      });
    },
    draw(id, ref) {
      const rectHeight = 10;
